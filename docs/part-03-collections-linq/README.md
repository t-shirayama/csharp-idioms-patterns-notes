...
- レビューではチェックリストを先に使い、順序、重複、null、キー重複、複数回列挙、DB 変換のどこにリスクがあるかをコメントに残す。
- 実装を変える前に、`List<T>` で十分な少件数なのか、索引化や materialize が必要な規模なのかを確認する。

## 項目一覧

- [コードレビュー用チェックリスト](checklist.md)
- [array、List、Dictionary、HashSet](array-list-dictionary-hashset.md)
- [IEnumerable / ICollection / IReadOnlyCollection / IReadOnlyList の使い分け](ienumerable-icollection-ireadonlycollection-の使い分け.md)
