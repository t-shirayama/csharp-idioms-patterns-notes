...
`LangVersion` を明示しない場合、既定の C# バージョンは TFM と連動します。たとえば .NET 8 は C# 12、.NET 9 は C# 13、.NET 10 は C# 14 が既定です。新構文を採用するときは、ローカル SDK だけでなく、`TargetFramework`、CI の SDK、IDE / analyzer の対応も合わせて確認します。
...
- `LangVersion` を明示する場合、その理由と対象 TFM の既定値との差分を説明できるか
...
- CI、コンテナ、ローカルで同じ SDK / TFM 前提でビルドとテストが通るか
- 設定 binding、validation、ログ出力が、アプリケーション種別に合った仕組みで差し替え可能か
- 新しい .NET API を使う場合、対象 TFM と運用 Runtime で利用できることを確認しているか
- ライブラリの公開 API が、利用側の TFM 制約を不要に引き上げていないか
...
## TODO対応: SDK / Runtime / TFM に加え

> 対応元: P0 / SDK / Runtime / TFM に加え、LTS/STS、`global.json`、CI SDK 固定、multi-targeting、nullable/analyzer を warnings as errors にする判断を追加する。 / 実務の導入判断に必要な運用・保守観点を補う。
